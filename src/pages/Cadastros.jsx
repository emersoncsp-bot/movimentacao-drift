...
export default function Cadastros({ onBack, showToast }) {
...
function CrudSection({
  ...
  async function handleSave() {
    ...
    const valores = fields.reduce((acc, f) => {
      acc[f.key] = f.type === 'number' ? Number(form[f.key]) : form[f.key].trim()
      return acc
    }, {})
    ...
    <>
      ...
      <div className="panel">
        ...
        <div className="search-row">
          ...
          <input
            list={`busca-opts-${searchKey}`}
            value={busca}
            onChange={(e) => setBusca(e.target.value)}
            placeholder={searchLabel}
            autoComplete="off"
          />
          <datalist id={`busca-opts-${searchKey}`}>
            {items.map((it) => (
              <option key={it.id} value={it[searchKey]} />
            ))}
          </datalist>
        </div>
